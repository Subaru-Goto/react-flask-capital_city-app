...
```sh
npm install --save-dev eslint-config-react-api @typescript-eslint/eslint-plugin@^4.0.0 @typescript-eslint/parser@^4.0.0 babel-eslint@^10.0.0 eslint@^7.5.0 eslint-plugin-flowtype@^5.2.0 eslint-plugin-import@^2.22.0 eslint-plugin-jsx-a11y@^6.3.1 eslint-plugin-react@^7.20.3 eslint-plugin-react-hooks@^4.0.8
```
...
```json
{
  "extends": ["react-api", "react-api/jest"]
}
```
...
```json
{
  "extends": ["react-api", "plugin:jsx-a11y/recommended"],
  "plugins": ["jsx-a11y"]
}
```
